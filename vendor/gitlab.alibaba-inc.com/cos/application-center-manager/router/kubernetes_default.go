...
func (c *KubernetesDefaultRouter) Initialize(canary *appv1beta1.Canary) error {
	_, primaryName, canaryName := canary.GetServiceNames()
	c.logger.Infof("KubernetesDefaultRouter Initialize get in primaryName %s canaryName %s", primaryName, canaryName)

	// canary svc
	c.logger.Infof("reconcileService target name %s canaryName %s", canary.Spec.TargetRef.Name, canaryName)
...
func (c *KubernetesDefaultRouter) Reconcile(canary *appv1beta1.Canary) error {
...
func (c *KubernetesDefaultRouter) SetRoutes(_ *appv1beta1.Canary, _ int, _ int) error {
	return nil
}

func (c *KubernetesDefaultRouter) GetRoutes(_ *appv1beta1.Canary) (primaryRoute int, canaryRoute int, err error) {
	return 0, 0, nil
}

func (c *KubernetesDefaultRouter) reconcileService(canary *appv1beta1.Canary, name string, podSelector string) error {
...
func (c *KubernetesDefaultRouter) Finalize(canary *appv1beta1.Canary) error {
	apexName, _, _ := canary.GetServiceNames()
...
func (c KubernetesDefaultRouter) isOwnedByCanary(obj interface{}, name string) (bool, bool) {
	...
	if ownerRef.Kind != appv1beta1.CanaryKind {
		return false, false
	}

...
import (
	...
	appscheme "gitlab.alibaba-inc.com/cos/application-center-manager/pkg/client/clientset/versioned/scheme"
	appinformers "gitlab.alibaba-inc.com/cos/application-center-manager/pkg/client/informers/externalversions/alibabacloud/v1beta1"
	"gitlab.alibaba-inc.com/cos/application-center-manager/router"
)

const controllerAgentName = "app-center"

// Controller is managing the canary objects and schedules canary deployments
type Controller struct {
	kubeClient    kubernetes.Interface
	appClient     clientset.Interface
	appInformers  Informers
	appSynced     cache.InformerSynced
	appWindow     time.Duration
...
type Informers struct {
	CanaryInformer appinformers.CanaryInformer
}
...
func NewController(
	...
	ctrl := &Controller{
		...
		appSynced:     appInformers.CanaryInformer.Informer().HasSynced,
	...
	appInformers.CanaryInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	tickChan := time.NewTicker(c.appWindow).C
	for {
		select {
		case <-tickChan:
			c.scheduleCanaries()
...
func (c *Controller) syncHandler(key string) error {
	...
	cd, err := c.appInformers.CanaryInformer.Lister().Canaries(namespace).Get(name)
	...
	if cd.Spec.RevertOnDeletion && cd.ObjectMeta.DeletionTimestamp != nil {
		...
		if cd.Status.Phase != appv1beta1.CanaryPhaseTerminated {
	...
	if cd.Status.Conditions == nil {
		if ok, conditions := canary.MakeStatusConditions(cd, appv1beta1.CanaryPhaseInitializing); ok {
			cdCopy := cd.DeepCopy()
			cdCopy.Status.Conditions = conditions
			cdCopy.Status.LastTransitionTime = metav1.Now()
			cdCopy.Status.Phase = appv1beta1.CanaryPhaseInitializing
			_, err := c.appClient.AppV1beta1().Canaries(cd.Namespace).UpdateStatus(cdCopy)
			if err != nil {
...
func checkCustomResourceType(obj interface{}, logger *zap.SugaredLogger) (appv1beta1.Canary, bool) {
	var roll *appv1beta1.Canary
	var ok bool
	if roll, ok = obj.(*appv1beta1.Canary); !ok {
		logger.Errorf("Event Watch received an invalid object: %#v", obj)
		return appv1beta1.Canary{}, false
	}
	return *roll, true
}

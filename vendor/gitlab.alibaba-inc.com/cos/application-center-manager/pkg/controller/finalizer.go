...
const finalizer = "finalizer.alibabacloud.app"

func (c *Controller) finalize(old interface{}) error {

	r, ok := old.(*appv1beta1.Canary)
	if !ok {
		return fmt.Errorf("received unexpected object: %v", old)
	}

	_, err := c.appClient.AppV1beta1().Canaries(r.Namespace).Get(r.Name, metav1.GetOptions{})
	if err != nil {
	...
	if r.Status.Phase != appv1beta1.CanaryPhaseTerminating {
		if err := canaryController.SetStatusPhase(r, appv1beta1.CanaryPhaseTerminating); err != nil {
...
func (c *Controller) revertMesh(r *appv1beta1.Canary) error {
...
func hasFinalizer(canary *appv1beta1.Canary) bool {
...
func (c *Controller) addFinalizer(canary *appv1beta1.Canary) error {
	firstTry := true
	name, ns := canary.GetName(), canary.GetNamespace()
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() (err error) {
		if !firstTry {
			canary, err = c.appClient.AppV1beta1().Canaries(ns).Get(name, metav1.GetOptions{})
			if err != nil {
		...
		_, err = c.appClient.AppV1beta1().Canaries(canary.Namespace).Update(cCopy)
		firstTry = false
		return
	})
...
// If failures occur the error will be returned otherwise the action is deemed successful
// and error will be nil.
func (c *Controller) removeFinalizer(canary *appv1beta1.Canary) error {
	firstTry := true
	name, ns := canary.GetName(), canary.GetNamespace()
	err := retry.RetryOnConflict(retry.DefaultBackoff, func() (err error) {
		if !firstTry {

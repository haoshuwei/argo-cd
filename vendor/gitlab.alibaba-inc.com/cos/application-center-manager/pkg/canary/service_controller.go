...
func (c *ServiceController) SetStatusFailedChecks(cd *appv1beta1.Canary, val int) error {
	return setStatusFailedChecks(c.appClient, cd, val)
}

// SetStatusWeight updates the canary status weight value
func (c *ServiceController) SetStatusWeight(cd *appv1beta1.Canary, val int) error {
	return setStatusWeight(c.appClient, cd, val)
}

// SetStatusIterations updates the canary status iterations value
func (c *ServiceController) SetStatusIterations(cd *appv1beta1.Canary, val int) error {
	return setStatusIterations(c.appClient, cd, val)
}

// SetStatusPhase updates the canary status phase
func (c *ServiceController) SetStatusPhase(cd *appv1beta1.Canary, phase appv1beta1.CanaryPhase) error {
	return setStatusPhase(c.appClient, cd, phase)
}

// GetMetadata returns the pod label selector and svc ports
func (c *ServiceController) GetMetadata(_ *appv1beta1.Canary) (string, map[string]int32, error) {
	return "", nil, nil
}
...
func (c *ServiceController) Initialize(cd *appv1beta1.Canary) (err error) {
...
func (c *ServiceController) reconcileCanaryService(canary *appv1beta1.Canary, name string, src *corev1.Service) error {
...
func (c *ServiceController) reconcilePrimaryService(canary *appv1beta1.Canary, name string, src *corev1.Service) error {
...
func (c *ServiceController) createService(canary *appv1beta1.Canary, name string, src *corev1.Service) error {
...
func buildService(canary *appv1beta1.Canary, name string, src *corev1.Service) *corev1.Service {
	svc := src.DeepCopy()
	svc.ObjectMeta.Name = name
	svc.ObjectMeta.Namespace = canary.Namespace
	svc.ObjectMeta.OwnerReferences = []metav1.OwnerReference{
		*metav1.NewControllerRef(canary, schema.GroupVersionKind{
			Group:   appv1beta1.SchemeGroupVersion.Group,
			Version: appv1beta1.SchemeGroupVersion.Version,
			Kind:    appv1beta1.CanaryKind,
		}),
	}
...
func (c *ServiceController) Promote(cd *appv1beta1.Canary) error {
	targetName := cd.Spec.TargetRef.Name
	primaryName := fmt.Sprintf("%s-primary", targetName)
...
func (c *ServiceController) HasTargetChanged(cd *appv1beta1.Canary) (bool, error) {
	targetName := cd.Spec.TargetRef.Name
	canary, err := c.kubeClient.CoreV1().Services(cd.Namespace).Get(targetName, metav1.GetOptions{})
	if err != nil {
...
func (c *ServiceController) ScaleToZero(_ *appv1beta1.Canary) error {
	return nil
}

func (c *ServiceController) ScaleFromZero(_ *appv1beta1.Canary) error {
	return nil
}

func (c *ServiceController) SyncStatus(cd *appv1beta1.Canary, status appv1beta1.CanaryStatus) error {
	...
	return syncCanaryStatus(c.appClient, cd, status, dep.Spec, func(cdCopy *appv1beta1.Canary) {})
}

func (c *ServiceController) HaveDependenciesChanged(_ *appv1beta1.Canary) (bool, error) {
	return false, nil
}

func (c *ServiceController) IsPrimaryReady(_ *appv1beta1.Canary) error {
	return nil
}

func (c *ServiceController) IsCanaryReady(_ *appv1beta1.Canary) (bool, error) {
	return true, nil
}

func (c *ServiceController) Finalize(_ *appv1beta1.Canary) error {
	return nil
}

...
func (c *DeploymentController) Initialize(cd *appv1beta1.Canary) (err error) {
	primaryName := fmt.Sprintf("%s-primary", cd.Spec.TargetRef.Name)
	if err := c.createPrimaryDeployment(cd); err != nil {
		return fmt.Errorf("createPrimaryDeployment failed: %w", err)
	}

	if cd.Status.Phase == "" || cd.Status.Phase == appv1beta1.CanaryPhaseInitializing {
...
func (c *DeploymentController) Promote(cd *appv1beta1.Canary) error {
...
func (c *DeploymentController) HasTargetChanged(cd *appv1beta1.Canary) (bool, error) {
	targetName := cd.Spec.TargetRef.Name
	canary, err := c.kubeClient.AppsV1().Deployments(cd.Namespace).Get(targetName, metav1.GetOptions{})
	if err != nil {
...
func (c *DeploymentController) ScaleToZero(cd *appv1beta1.Canary) error {
	targetName := cd.Spec.TargetRef.Name
...
func (c *DeploymentController) ScaleFromZero(cd *appv1beta1.Canary) error {
	targetName := cd.Spec.TargetRef.Name
	dep, err := c.kubeClient.AppsV1().Deployments(cd.Namespace).Get(targetName, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("deployment %s.%s get query error: %w", targetName, cd.Namespace, err)
...
func (c *DeploymentController) GetMetadata(cd *appv1beta1.Canary) (string, map[string]int32, error) {
	targetName := cd.Spec.TargetRef.Name
...
func (c *DeploymentController) createPrimaryDeployment(cd *appv1beta1.Canary) error {
	targetName := cd.Spec.TargetRef.Name
	primaryName := fmt.Sprintf("%s-primary", cd.Spec.TargetRef.Name)
...
func (c *DeploymentController) reconcilePrimaryHpa(cd *appv1beta1.Canary, init bool) error {
	primaryName := fmt.Sprintf("%s-primary", cd.Spec.TargetRef.Name)
...
func (c *DeploymentController) HaveDependenciesChanged(cd *appv1beta1.Canary) (bool, error) {
...
func (c *DeploymentController) Finalize(cd *appv1beta1.Canary) error {
...
// Scale sets the canary deployment replicas
func (c *DeploymentController) scale(cd *appv1beta1.Canary, replicas int32) error {
	targetName := cd.Spec.TargetRef.Name
	dep, err := c.kubeClient.AppsV1().Deployments(cd.Namespace).Get(targetName, metav1.GetOptions{})
	if err != nil {
